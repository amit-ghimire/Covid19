...
public class DataVisualizer : MonoBehaviour {
    ...
    public float ValueScaleMultiplier = 2e-05f;
    public float ColorScaleMultiplier = 0.00025f;

    public TextMeshProUGUI tmpConfirmedTotal;
    public TextMeshProUGUI tmpActiveTotal;
    public TextMeshProUGUI tmpRecoveredTotal;
    public TextMeshProUGUI tmpFatalTotal;
    public TextMeshProUGUI tmpTimeStamp;
    ...
    public IEnumerator LoadJSONDataFromWeb()
    {
        ...
        else
        {
            ...
            {
                Debug.LogError("Could not get data");
            }
        } 
    ...
    public void CreateMeshes()
    {
        ...
        for (int i = 0; i < featuresArray.Count; i++)
        {
            ...
            CreateObject(meshVertices, meshIndices, meshColors, seriesObj, locationName, n_confirmed, n_recovered, n_deaths, GraphContainerConfirmed);
            meshVertices.Clear();
            meshIndices.Clear();
            meshColors.Clear();


            CreateObject(meshVertices, meshIndices, meshColors, seriesObj, locationName, n_confirmed, n_recovered, n_deaths, GraphContainerRecovered);
            meshVertices.Clear();
            meshIndices.Clear();
            meshColors.Clear();

            CreateObject(meshVertices, meshIndices, meshColors, seriesObj, locationName, n_confirmed, n_recovered, n_deaths, GraphContainerFatal);
            meshVertices.Clear();
            meshIndices.Clear();
            meshColors.Clear();
    ...
    string locationName, GameObject containerObject, bool createLabel)
    {
        ...
        Color valueColor = Colors.Evaluate(value * ColorScaleMultiplier);
        Vector3 pos;
    ...
    private void CreateObject(List<Vector3> meshertices, List<int> meshindecies, List<Color> meshColors, GameObject seriesObj,
        string locationName,int n_confirmed,int n_recovered, int n_deaths,
        GameObject containerObject)
    {
        if (locationName != "Recovered")
        {
            Mesh mesh = new Mesh();
            ...
            if (obj.GetComponent<CovidData>() == null)
            {
                ...
                obj.GetComponent<CovidData>().recovered = n_recovered;
                obj.GetComponent<CovidData>().fatal = n_deaths;
            }
        }
    }
